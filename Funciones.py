...
def distanceP(velocity, density, protons, neutrons, particula):
    if particula == "Positrón":
        return abs(((int(protons)+int(neutrons))*(9.11*10**-31)*int(velocity)**2*(8.85*10**-12))/(int(protons)*(1.6*10**-19)*float(density)))
    else :
        return abs(((int(protons)+int(neutrons))*(1.67*10**-27)*int(velocity)**2*(8.85*10**-12))/(int(protons)*(1.6*10**-19)*float(density)))

#Distancia esfera
def distanceS(radio, protons, neutrons, velocity, particula, cargaEsfera):
    if particula == "Positrón":
        return abs((2*math.pi*(8.85*10**-12)*float(radio)**2*(int(protons)+int(neutrons))*(9.11*10**-31)*int(velocity)**2)/(int(protons)*(1.6*10**-19))*float(cargaEsfera))
    else :
        return abs((2*math.pi*(8.85*10**-12)*float(radio)**2*(int(protons)+int(neutrons))*(1.67*10**-27)*int(velocity)**2)/(int(protons)*(1.6*10**-19))*float(cargaEsfera))
...
def distanceSP(radio, masa, velocity, carga, cargaEsfera):
    return abs((2*math.pi*(8.85*10**-12)*float(radio)**2*float(masa)*int(velocity)**2)/float(carga)*float(cargaEsfera))
